    ...
    <div class="container">
        ...
        <div class="main-content">
            <div class="card">
                ...
                <form method="POST" action="{{ route('dashboard.track') }}">
                    @csrf
            ...
            <div class="card">
                ...
                <div class="tabs">
                    <button class="tab active" onclick="switchTab('active')">
                        En Tránsito <span class="tab-count">{{ $activeShipments->count() }}</span>
                    </button>
                    <button class="tab" onclick="switchTab('pending')">
                        En Verificación <span class="tab-count">{{ $pendingTrackings->count() }}</span>
                    </button>
                    <button class="tab" onclick="switchTab('delivered')">
                        Historial <span class="tab-count">{{ $deliveredShipments->count() }}</span>
                    </button>
                </div>
                    ...
                    <div id="shipments-list-active" class="shipments-list">
                        @if ($activeShipments->count() > 0)
                            @foreach ($activeShipments as $shipment)
                                @include('dashboard.partials.shipment-item', ['shipment' => $shipment])
                            @endforeach
                        @else
                ...
                <div id="tab-pending" class="tab-content">
                    <div id="shipments-list-pending" class="shipments-list">
                        @if ($pendingTrackings->count() > 0)
                            @foreach ($pendingTrackings as $pending)
                                <div class="shipment-item" style="border-left: 4px solid #ff751f;">
                                    <div class="shipment-header">
                                        <div class="tracking-number">{{ $pending->tracking_number }}</div>
                                        <div style="display: flex; gap: 10px; align-items: center;">
                                            <span class="status-badge" style="background: #fff3cd; color: #856404;">
                                                ⏳ En Verificación
                                            </span>
                                            <form action="{{ route('pending-tracking.delete', $pending->id) }}" method="POST" style="display: inline;" onsubmit="return confirm('¿Estás seguro de eliminar este tracking de verificación?')">
                                                @csrf
                                                @method('DELETE')
                                    ...
                                    <div class="shipment-details">
                                        ...
                                        <div class="detail-item">
                                            ...
                                            <span class="detail-value">
                                                {{ $pending->attempts }} verificación(es)
                                            </span>
                                        </div>
                                    </div>
                                </div>
